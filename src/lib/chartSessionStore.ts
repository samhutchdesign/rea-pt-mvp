...
export function signChartSession(patientId: string, sessionId: string, signer: { empId: string; name: string; initials: string; signatureFontId: string; date?: string }): void {
  const current = getChartSessions(patientId);
  _state = new Map(_state).set(
    patientId,
    current.map((s) => (s.id === sessionId
      ? { ...s, ...(signer.date ? { date: signer.date } : {}), signedAt: new Date().toISOString(), signedByEmpId: signer.empId, signedByName: signer.name, signedByInitials: signer.initials, signatureFontId: signer.signatureFontId }
      : s
    ))
  );
  notify(patientId);
