...
export default function ProgramEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [maxReachedStep, setMaxReachedStep] = useState(existingProgram ? 2 : 0);
  ...
  const [imageUrl, setImageUrl] = useState<string | null>(existingProgram?.imageUrl ?? null);
  ...
  const handleSave = () => {
    ...
    if (progIdx !== -1) {
      mockPrograms[progIdx] = { ...mockPrograms[progIdx], name: newProgramName, description: description.trim(), frequency, exercises: newExercises, imageUrl: imageUrl ?? undefined };
    } else {
      mockPrograms.push({
        id: newProgramId,
        name: newProgramName,
