...
export default function DocumentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!doc)
    return (
      <div className="p-8">
        <span className="text-tertiary text-xs">Document not found.</span>
      </div>
    );
        ...
        <div className="flex flex-col gap-5">
          {doc.fields.map((field) => (
            <div key={field.id}>
              ...
              {field.type === 'dropdown' ? (
                <div className="border border-secondary rounded-lg px-3 py-2 bg-secondary text-tertiary text-xs">
                ...
                <div className="border border-secondary rounded-lg px-3 py-2 bg-secondary flex items-center gap-2">
                  ...
                  <span className="text-tertiary text-xs">{field.label}</span>
                </div>
              ) : field.type === 'textarea' ? (
                <textarea
                  rows={3}
                  ...
                  className="w-full rounded-lg border border-secondary bg-secondary px-3 py-2 text-xs text-tertiary placeholder:text-placeholder outline-none resize-none"
                />
              ) : (
                <input
                  readOnly
                  ...
                  className="w-full rounded-lg border border-secondary bg-secondary px-3 py-2 text-xs text-tertiary placeholder:text-placeholder outline-none"
                />
              )}
            </div>
          ))}
