...
export function ChartTemplateSelect({ onSelect }: { onSelect: (id: ChartTemplateId) => void }) {
  return (
    <div className="max-w-[820px] mx-auto">
      ...
      <p className="mb-6 text-xs text-tertiary">Choose the chart template to use for this session.</p>
        ...
        {CHART_TEMPLATES.map((template) => (
          ...
          >
            <div className="flex w-full items-start justify-between gap-2">
              <span className="text-base font-semibold text-primary">{template.name}</span>
              {template.comingSoon ? (
